...
it reads naturally. Constraint text must use the same readable weight and color
as the statement body, not faint muted helper text. Questions whose official
statement does not separate data ranges, such as some database tasks, receive
an explicit note to use the table schema or input definition in the statement.
