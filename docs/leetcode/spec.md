...
separate bordered block or bullet list. Questions whose official statement does
not separate data ranges, such as some database tasks, receive an explicit note
to use the table schema or input definition in the statement.
...
as math instead of broken plain text such as `n 2`, `m x n`, or `10 9`. The
page reads only the generated escaped HTML and imports KaTeX CSS globally.
`npm run check:leetcode-math` must pass before publishing math or statement
changes.
