...
<figure class="wc-article-figure">
  <img src="https://cdn.jsdelivr.net/gh/WineChord/typora-images/img/cloud-task-remote-work-contract-df64b03.webp" alt="Cloud task contract linking remote identity, environment selection, attempts, diff retrieval, and local apply verification" loading="eager" decoding="async" />
  <figcaption>Cloud tasks are remote work contracts with identity, environment, attempts, diff retrieval, and local apply verification.</figcaption>
</figure>
...
<figure class="wc-article-figure">
  <img src="https://cdn.jsdelivr.net/gh/WineChord/typora-images/img/provider-runtime-split-569ff6a1.webp" alt="Turn sampling response events entering a runtime boundary and splitting toward realtime sideband, backend tasks, task lifecycle, and apply diff" loading="lazy" decoding="async" width="1672" height="941" />
  <figcaption>Cloud task work branches away from local turn streaming; it returns through task state and applyable diff contracts.</figcaption>
</figure>

**Source boundary:** the backend task surface is **verified source** in [`cloud-tasks-client/src/api.rs`](https://github.com/openai/codex/blob/569ff6a1c400bd514ff79f5f1050a684dc3afde3/codex-rs/cloud-tasks-client/src/api.rs#L22), HTTP transport is **verified source** in [`http.rs`](https://github.com/openai/codex/blob/569ff6a1c400bd514ff79f5f1050a684dc3afde3/codex-rs/cloud-tasks-client/src/http.rs#L24), and the mock implementation is **verified source** in [`cloud-tasks-mock-client`](https://github.com/openai/codex/blob/569ff6a1c400bd514ff79f5f1050a684dc3afde3/codex-rs/cloud-tasks-mock-client/src/mock.rs#L18). Claims that connect task identity, remote workflow, and local apply are **surrounding contract inference** from those client shapes, [`agent-identity`](https://github.com/openai/codex/blob/569ff6a1c400bd514ff79f5f1050a684dc3afde3/codex-rs/agent-identity/src/lib.rs#L40), and the local diff discipline in [`turn_diff_tracker.rs`](https://github.com/openai/codex/blob/569ff6a1c400bd514ff79f5f1050a684dc3afde3/codex-rs/core/src/turn_diff_tracker.rs#L18).
...
<figure class="wc-article-figure">
  <img src="https://cdn.jsdelivr.net/gh/WineChord/typora-images/img/patch-delta-evidence-569ff6a1.webp" alt="Committed patch deltas entering a turn diff tracker, with exact evidence rendering a diff and uncertain evidence invalidating the display" loading="lazy" decoding="async" width="1672" height="941" />
  <figcaption>Remote completion does not mutate the checkout; local diff evidence and preflight decide what can be applied.</figcaption>
</figure>
...
<figure class="wc-article-figure">
  <img src="https://cdn.jsdelivr.net/gh/WineChord/typora-images/img/task-scoped-identity-trust-path-20260616.webp" alt="Task-scoped identity trust path separating user authentication, runtime key registration, signed task authorization, task id, JWT claims, bill of materials, and backend task boundary" loading="lazy" decoding="async" width="1672" height="941" />
  <figcaption>Task-scoped identity keeps account context separate from the runtime key that is authorized to act for one backend task.</figcaption>
</figure>
...
| Question | Chapter 21 answer |
| --- | --- |
| Where is the user request now? | It has crossed from a local prompt into a backend task contract, then may return as task text, messages, attempts, or a diff. |
| What carries it? | `TaskId`, `TaskSummary`, `TaskText`, `TurnAttempt`, environment id, git ref, signed task identity, and optional diff override. |
| Who decides next? | Backend state decides task progress; local Codex decides environment selection, attempt selection, preflight, and whether a returned diff mutates the checkout. |
| What can fail here? | missing auth, ambiguous environment, branch fallback, stale remote diff, absent attempt diff, partial apply, identity mismatch, or unverifiable JWT material. |
