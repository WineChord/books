...
The interactive model below maps those phrases onto four paths: intent entering
as an operation, model output returning as items, authority deciding side
effects, and facts becoming rollout evidence. Step through it once, then switch
to the authority stack. The most important path is still the one the component
refuses to draw: model output never reaches shell or filesystem directly.

<BoundedAgentOSMap lang="en" client:visible />

Notice the distinction the component keeps making. Operations are requests
entering the runtime; events are facts leaving it. Clients can change, but the
operation/event contract and the runtime boundary remain stable.
...
<figure className="sketch-figure">
  <img src="/books/figures/codex-from-source/excalidraw/chapter-01-concept-1-en.svg" alt="Thread, turn, item, operation, event, tool, and rollout name the runtime facts that keep user intent, model output, side effects, and replay from collapsing into one transcript." loading="lazy" />
  <figcaption>Thread, turn, item, operation, event, tool, and rollout name the runtime facts that keep user intent, model output, side effects, and replay from collapsing into one transcript.</figcaption>
</figure>
...
<figure className="sketch-figure">
  <img src="/books/figures/codex-from-source/excalidraw/chapter-01-concept-2-en.svg" alt="Contracts come first because clients are replaceable surfaces; typed operations and accepted events are the stable boundary that keeps runtime authority inside the system." loading="lazy" />
  <figcaption>Contracts come first because clients are replaceable surfaces; typed operations and accepted events are the stable boundary that keeps runtime authority inside the system.</figcaption>
</figure>
...
<figure className="sketch-figure">
  <img src="/books/figures/codex-from-source/excalidraw/chapter-01-concept-3-en.svg" alt="A turn behaves like a system call: typed intent enters the runtime, policy decides which effects may happen, and accepted events become the evidence that replay can trust." loading="lazy" />
  <figcaption>A turn behaves like a system call: typed intent enters the runtime, policy decides which effects may happen, and accepted events become the evidence that replay can trust.</figcaption>
</figure>
...
<figure className="sketch-figure">
  <img src="/books/figures/codex-from-source/excalidraw/chapter-01-concept-4-en.svg" alt="Authority is layered so no single model message can become power: clients submit intent, runtime policy gates it, tool handlers execute narrow effects, and rollout evidence records what happened." loading="lazy" />
  <figcaption>Authority is layered so no single model message can become power: clients submit intent, runtime policy gates it, tool handlers execute narrow effects, and rollout evidence records what happened.</figcaption>
</figure>
...
Switch the model above to **Authority stack** and walk it from tool
specification through sandbox and executor. That view is the important
replacement for the old static stack: every layer can narrow, block, delay, or
explain the request before any observed result exists.
