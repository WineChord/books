...
> **Reading Contract:** Use this chapter to follow artifacts from Rust workspace to distributed package. Track native binaries, npm wrappers, release gates, and platform-specific dependency boundaries.

<figure class="wc-article-figure">
  <img src="https://cdn.jsdelivr.net/gh/WineChord/typora-images/img/release-artifact-conveyor.webp" alt="Release artifact conveyor moving Rust workspace outputs through native binaries, npm wrappers, platform packages, signatures, and checksums" loading="eager" decoding="async" />
  <figcaption>Release work is a conveyor: Rust crates, native binaries, wrappers, packages, platform checks, signing, versions, and smoke tests move together.</figcaption>
</figure>
...
<figure class="wc-article-figure">
  <img src="https://cdn.jsdelivr.net/gh/WineChord/typora-images/img/npm-native-payload-selection-20260616.webp" alt="NPM native payload selection lifecycle from meta package through platform option, wrapper, Rust binary, diagnostic fallback, and platform facts" loading="lazy" decoding="async" width="1672" height="941" />
  <figcaption>The npm-facing layer selects a native payload at the delivery boundary; the wrapper reports platform problems instead of becoming runtime logic.</figcaption>
</figure>
...
<figure class="wc-article-figure">
  <img src="https://cdn.jsdelivr.net/gh/WineChord/typora-images/img/native-dependency-quarantine-trust-20260616.webp" alt="Native dependency quarantine and trust transfer separating native inputs, patches, checksums, helper package, build artifact, signed artifact, manifest, installer, and runtime clean boundary" loading="lazy" decoding="async" width="1672" height="941" />
  <figcaption>Native dependency quarantine keeps archive, patch, checksum, helper, signing, and installer concerns visible in release engineering instead of leaking into the turn loop.</figcaption>
</figure>
...
| Question | Chapter 24 answer |
| --- | --- |
| Where is the user request now? | It has become an installable command path: native binary, wrapper package, optional platform payload, installer, or release artifact. |
| What carries it? | package metadata, staged npm files, platform-specific native components, vendor payloads, release workflows, checksums, signatures, and helper dependency rules. |
| Who decides next? | Release scripts and package selectors decide which artifact reaches the user; the Rust binary remains the owner of runtime behavior after launch. |
| What can fail here? | missing native component, wrong platform package, stale vendor archive, unsigned or unchecked payload, opaque optional dependency failure, or installer-only behavior drift. |
