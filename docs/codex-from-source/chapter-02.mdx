...
import { startupChain, routerToFoundation } from "../../src/data/diagrams/codex-from-source/chapter-02";

# Chapter 2: From Distribution Wrapper to Rust Router

<StartupBootstrapTimeline lang="en" client:visible />

> **Reading Contract:** Use this chapter to follow startup as narrowing. Track how distribution wrappers, CLI parsing, and Rust routing turn a user command into a typed path without letting packaging own architecture.

<figure class="wc-article-figure">
  <img src="/books/codex-from-source/assets/startup-router-narrowing.png" alt="chapter 02 hand-drawn architecture figure" loading="lazy" decoding="async" />
  <figcaption>Startup narrows a broad command into one typed runtime path: wrapper, Rust CLI, command router, then the selected surface.</figcaption>
</figure>
...
<DiagramFlow spec={startupChain} lang="en" client:visible />
...
<DiagramFlow spec={routerToFoundation} lang="en" client:visible />
