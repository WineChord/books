...
> **Reading Contract:** Use this chapter to read CI as executable architecture. Track policy lanes, test targets, generated checks, and the failures that protect runtime contracts.

<figure class="wc-article-figure">
  <img src="https://cdn.jsdelivr.net/gh/WineChord/typora-images/img/ci-policy-governance-lanes-df64b03.webp" alt="CI governance lanes connecting policy checks, generated artifacts, Rust tests, Bazel verification, and release protection" loading="eager" decoding="async" />
  <figcaption>CI protects architecture by separating behavior tests, boundary linting, schema checks, artifact checks, risk policy, and docs evidence.</figcaption>
</figure>
...
<figure class="wc-article-figure">
  <img src="https://cdn.jsdelivr.net/gh/WineChord/typora-images/img/generated-artifact-drift-state-machine-20260616.webp" alt="Generated artifact drift state machine from source type change through generator output, artifact diff, CI gate, fail regenerate path, review path, and pass contract change" loading="lazy" decoding="async" width="1672" height="941" />
  <figcaption>Generated drift becomes governance only when the repository turns changed source types into visible diffs, CI outcomes, repair paths, and reviewable contract changes.</figcaption>
</figure>
...
| Question | Chapter 25 answer |
| --- | --- |
| Where is the user request now? | It has become repository change pressure: a protocol edit, dependency edit, binary artifact, UI import, generated output, or release workflow change. |
| What carries it? | workflow lanes, generated artifact checks, blob-size policy, dependency policy, boundary scripts, test matrices, and release gates. |
| Who decides next? | CI cannot judge design taste, but it can block changes that violate selected architecture invariants before review becomes memory work. |
| What can fail here? | over-broad gates, stale policy scripts, missing generated artifacts, direct TUI/core imports, unexplained large blobs, or checks that fail without a repair path. |
