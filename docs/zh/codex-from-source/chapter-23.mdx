...
> **阅读契约：** 用本章连接 generated contracts 与治理。跟住 schemas、bindings、drift detection 和让 protocol changes 可见的 build checks。

<figure class="wc-article-figure">
  <img src="https://cdn.jsdelivr.net/gh/WineChord/typora-images/img/generated-contract-drift-check.webp" alt="生成契约漂移检查：连接 Rust 类型、导出 schema、已提交产物、CI 失败和客户端 API 审核" loading="eager" decoding="async" />
  <figcaption>当 source schema、bindings、drift checks 和 CI failure 指向同一契约时，生成 schema 就成了治理机制。</figcaption>
</figure>
...
第 22 章仍然在 runtime 内部：长期记忆只能通过 read path、write path、lock、 citation 和受限 internal agent 改变。第七部把同样的纪律推进到仓库层面。 如果 runtime 由持久契约治理，source tree 也需要一套机器，确保每一次依赖变更、 schema 编辑、平台移植和 release build 之后，这些契约仍然成立。
...
<figure class="wc-article-figure">
  <img src="https://cdn.jsdelivr.net/gh/WineChord/typora-images/img/dual-build-governance-overview-20260616.webp" alt="Dual build governance overview：一个 Rust source graph 输入 Cargo、Bazel overlay、generated schemas、CI drift gate 与 release checks" loading="lazy" decoding="async" width="1672" height="941" />
  <figcaption>一个 source graph 同时服务开发构建、hermetic overlay、客户端 schema、drift gate 和 release check，但不制造第二套架构。</figcaption>
</figure>
...
<figure class="wc-article-figure">
  <img src="https://cdn.jsdelivr.net/gh/WineChord/typora-images/img/schema-governance-filter-569ff6a1.webp" alt="Generated schema governance path：Rust protocol types 经过 schema export、TypeScript、JSON Schema、experimental filter、clients 与 drift checks" loading="lazy" decoding="async" width="1672" height="941" />
  <figcaption>Schema generation 是 contract path：它把 Rust types 变成 client-facing artifacts 与 drift checks。</figcaption>
</figure>
...
| 问题 | 第 23 章答案 |
| --- | --- |
| 用户请求现在在哪里？ | 它已经离开 runtime execution，进入判断源码变化是否仍生成同一组公开承诺的 repository contract layer。 |
| 什么数据结构携带它？ | Cargo manifests、Bazel module 与 macros、schema exporters、已提交 JSON schemas、generated TypeScript 和 CI verification targets。 |
| 谁拥有下一步决策？ | 开发者修改 Rust types；generators 与 build checks 决定 client-facing artifacts、hermetic targets 和 committed outputs 是否仍然匹配。 |
| 这里可能怎么失败？ | generated files 过期、experimental fields 泄漏进 stable schemas、Cargo/Bazel drift、test launcher 失效、native dependency mismatch，或 boundary checks 不再覆盖真实风险。 |
