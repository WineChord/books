...
> **阅读契约：** 用本章跟踪 artifact 如何从 Rust workspace 到分发包。跟住 native binaries、npm wrappers、release gates 和平台相关依赖边界。

<figure class="wc-article-figure">
  <img src="https://cdn.jsdelivr.net/gh/WineChord/typora-images/img/release-artifact-conveyor.webp" alt="发布产物传送带：把 Rust workspace 输出送过 native binaries、npm wrappers、平台包、签名和 checksum" loading="eager" decoding="async" />
  <figcaption>Release 是传送带：Rust crates、native binaries、wrappers、packages、platform checks、signing、versions 和 smoke tests 一起移动。</figcaption>
</figure>
...
<figure class="wc-article-figure">
  <img src="https://cdn.jsdelivr.net/gh/WineChord/typora-images/img/npm-native-payload-selection-20260616.webp" alt="NPM native payload selection lifecycle：meta package、platform option、wrapper、Rust binary、diagnostic fallback 与 platform facts" loading="lazy" decoding="async" width="1672" height="941" />
  <figcaption>npm-facing layer 在 delivery boundary 选择 native payload；wrapper 负责诊断平台问题，而不是变成 runtime logic。</figcaption>
</figure>
...
<figure class="wc-article-figure">
  <img src="https://cdn.jsdelivr.net/gh/WineChord/typora-images/img/native-dependency-quarantine-trust-20260616.webp" alt="Native dependency quarantine and trust transfer：分开 native inputs、patches、checksums、helper package、build artifact、signed artifact、manifest、installer 与 runtime clean boundary" loading="lazy" decoding="async" width="1672" height="941" />
  <figcaption>Native dependency quarantine 让 archive、patch、checksum、helper、signing 和 installer concerns 留在 release engineering，而不是泄漏进 turn loop。</figcaption>
</figure>
...
| 问题 | 第 24 章答案 |
| --- | --- |
| 用户请求现在在哪里？ | 它已经变成可安装 command path：native binary、wrapper package、可选 platform payload、installer 或 release artifact。 |
| 什么数据结构携带它？ | package metadata、staged npm files、platform-specific native components、vendor payloads、release workflows、checksums、signatures 和 helper dependency rules。 |
| 谁拥有下一步决策？ | Release scripts 与 package selectors 决定哪个 artifact 到达用户；启动后 Rust binary 仍拥有 runtime behavior。 |
| 这里可能怎么失败？ | native component 缺失、platform package 错误、vendor archive 过期、payload 未签名或未校验、optional dependency failure 不可诊断，或 installer-only 行为漂移。 |
