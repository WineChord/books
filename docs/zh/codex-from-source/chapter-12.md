...
<div class="chapter-lede">
  ...
  <p><strong>问题：</strong>交互式终端和 headless app-server 需要不同呈现层，但不应该 fork agent runtime。</p>
  <p><strong>心智模型：</strong>core events 是河流；TUI 和 app-server 是从河流引出的两条渠道。</p>
</div>

TUI 和 app-server 不是事后补上的外壳。它们证明 runtime boundary 有价值。如果 core 只打印文本，外部 app 就得 scrape terminal output。如果 core 只服务 JSON-RPC，终端用户会失去丰富本地交互。Codex 通过结构化 runtime progress 同时支持两者。
...
<div class="flow">
  <div><strong>Core event</strong>Runtime 产生 progress、output、approval、diff 或 completion。</div>
  <div><strong>Surface adapter</strong>TUI 或 app-server 转成 presentation state。</div>
