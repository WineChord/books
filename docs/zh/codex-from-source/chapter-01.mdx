...
<figure class="wc-article-figure">
  <img src="/books/codex-from-source/assets/bounded-agent-os-map.png" alt="chapter 01 page hand-drawn architecture figure" loading="lazy" decoding="async" />
  <figcaption>把 Codex 看成有边界的运行环境更容易理解：客户端、类型协议、session runtime、权限、沙箱和 rollout 各自有 owner。</figcaption>
</figure>
...
下面这个交互模型把这些短语映射成四条路径：意图以 operation 进入，模型输出以 item 返回，权限层决定副作用，事实变成 rollout 证据。先走一遍 runtime 路径，再切到权限栈。最关键的路径仍然是组件拒绝画出的那条路径：模型输出永远不能直接抵达 shell 或文件系统。
...
把上面的模型切到 **权限栈**，从工具 spec 一直走到 sandbox 和 executor。这才是旧静态栈的真正替代：每一层都可以在出现观察结果之前收紧、阻断、延迟或解释这个请求。
