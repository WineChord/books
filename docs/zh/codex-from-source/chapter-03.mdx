...
> **阅读契约：** 用本章把配置读成约束。跟住 auth、policy、managed requirements、profiles 和环境事实如何变成后续代码必须服从的 envelope。

<figure class="wc-article-figure">
  <img src="https://cdn.jsdelivr.net/gh/WineChord/typora-images/img/config-auth-constraint-stack.webp" alt="配置与认证约束栈：展示 defaults、可信项目配置、auth snapshot、managed requirements 和 resolved permissions" loading="eager" decoding="async" />
  <figcaption>配置不是装饰项；defaults、profiles、auth、managed requirements 和 permissions 会编译成一次 turn 必须遵守的契约。</figcaption>
</figure>
...
**源码边界：** 关于 resolved permissions 的说法是 **verified source**，锚点在 [`config/mod.rs`](https://github.com/openai/codex/blob/569ff6a1c400bd514ff79f5f1050a684dc3afde3/codex-rs/core/src/config/mod.rs#L236)；permission compilation 是 **verified source**，锚点在 [`permissions.rs`](https://github.com/openai/codex/blob/569ff6a1c400bd514ff79f5f1050a684dc3afde3/codex-rs/core/src/config/permissions.rs#L300)；managed feature gates 是 **verified source**，锚点在 [`managed_features.rs`](https://github.com/openai/codex/blob/569ff6a1c400bd514ff79f5f1050a684dc3afde3/codex-rs/core/src/config/managed_features.rs#L25)。本章把它们称为 runtime envelope，是从这些 resolved shapes 和 [`app-server-protocol`](https://github.com/openai/codex/blob/569ff6a1c400bd514ff79f5f1050a684dc3afde3/codex-rs/app-server-protocol/src/protocol/v2/permissions.rs#L375) 的公开 permission profile 做出的 **surrounding contract inference**，不是对隐藏服务内部的断言。
