...
> **阅读契约：** 用本章把 memory 保持为受控旁路。跟住 read prompts、write prompts、user-level state、budgets，并记住 memory 不是隐藏 transcript。

<figure class="wc-article-figure">
  <img src="https://cdn.jsdelivr.net/gh/WineChord/typora-images/img/memory-side-channel-governance.webp" alt="Memory 旁路治理图：分开 read prompt、write pipeline、citation、lock、redaction、consolidation 与 storage" loading="eager" decoding="async" />
  <figcaption>Memory 是受控 side channel：reads、writes、budgets、locks、redaction、consolidation 和 storage 被有意拆开。</figcaption>
</figure>
...
**源码边界：** memory usage kinds 是 **verified source**，锚点在 [`usage.rs`](https://github.com/openai/codex/blob/569ff6a1c400bd514ff79f5f1050a684dc3afde3/codex-rs/memories/read/src/usage.rs#L8)；citation parsing 是 **verified source**，锚点在 [`citations.rs`](https://github.com/openai/codex/blob/569ff6a1c400bd514ff79f5f1050a684dc3afde3/codex-rs/memories/read/src/citations.rs#L6)；read-only service 与 write phases 是 **verified source**，锚点在 [`memories/mcp`](https://github.com/openai/codex/tree/569ff6a1c400bd514ff79f5f1050a684dc3afde3/codex-rs/memories/mcp/src)、[`phase1.rs`](https://github.com/openai/codex/blob/569ff6a1c400bd514ff79f5f1050a684dc3afde3/codex-rs/memories/write/src/phase1.rs#L70) 和 [`phase2.rs`](https://github.com/openai/codex/blob/569ff6a1c400bd514ff79f5f1050a684dc3afde3/codex-rs/memories/write/src/phase2.rs#L45)。本章的安全动机是从这些可见 rails 得出的 **surrounding contract inference**，不是对隐藏 memory service 行为的断言。
...
<figure class="wc-article-figure">
  <img src="https://cdn.jsdelivr.net/gh/WineChord/typora-images/img/memory-lifecycle-20260616.webp" alt="Memory lifecycle：capture、extract、store、retrieve、inject，以及 citation、audit、prune 和 feedback paths" loading="lazy" decoding="async" width="1672" height="941" />
  <figcaption>Memory side channel 有生命周期：capture、extract、store、retrieve、inject、cite、audit 与 prune 是不同操作。</figcaption>
</figure>
...
<figure class="wc-article-figure">
  <img src="https://cdn.jsdelivr.net/gh/WineChord/typora-images/img/memory-governance-boundaries-20260616.webp" alt="Memory governance boundaries：stale、secret、overreach、loop、authority risks 通过 consent、scope、redaction、citation、audit 控制" loading="lazy" decoding="async" width="1672" height="941" />
  <figcaption>Memory governance 解释了为什么 extraction、redaction、usage accounting、locks 与 restricted agents 必须分盒。</figcaption>
</figure>
...
| 问题 | 第 22 章答案 |
| --- | --- |
| 用户请求现在在哪里？ | 它可以读取受预算限制的长期 context，而 memory generation 作为 startup side pipeline 处理过去 rollouts。 |
| 什么数据结构携带它？ | memory summaries、citation entries、read-only MCP file operations、Stage 1 claims、Stage 1 outputs、workspace diffs 和 Stage 2 lock state。 |
| 谁拥有下一步决策？ | Active session 只能读取；memory pipeline 决定 extraction、redaction、retention、consolidation 与 workspace publication。 |
| 这里可能怎么失败？ | state DB 不可用、ephemeral sessions 被跳过、memory path 非法、symlink、empty query、extraction schema failure、lock contention 或 recursive consolidation。 |
