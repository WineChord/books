...
import { contractSurface, eventMapping } from "../../../src/data/diagrams/codex-from-source/chapter-14";

# 第 14 章：App-Server 契约

<JsonRpcContractMap lang="zh" client:visible />

> **阅读契约：** 用本章理解 app-server 作为 contract boundary。跟住 JSON-RPC methods、request serialization、event mapping，以及客户端如何重新接入而不拥有 runtime。

<figure class="wc-article-figure">
  <img src="/books/codex-from-source/assets/app-server-contract-bridge.png" alt="chapter 14 page hand-drawn architecture figure" loading="lazy" decoding="async" />
  <figcaption>App-server 是契约桥：JSON-RPC methods、request processors、thread runtime、event mapping 和 client views 保持对齐。</figcaption>
</figure>

第 13 章停在 runtime 边界：permissions、sandbox selection 和 managed networking 决定一次 tool attempt 可以触碰什么。本章再往外走一层。当 runtime 已经能够执行这些决策，它还需要一份契约，让 terminal client、SDK client、 daemon 管理的 client 和 remote client 共享同一套 thread，而不是共享同一份实现。
...
<DiagramFlow spec={contractSurface} lang="zh" client:visible />
...
<DiagramFlow spec={eventMapping} lang="zh" client:visible />
