...
<StartupBootstrapTimeline lang="zh" client:visible />

> **阅读契约：** 用本章把启动过程读成逐步收窄。跟住 distribution wrapper、CLI parsing 和 Rust routing 如何把用户命令变成类型化路径，同时不让 packaging 拥有架构。

<figure class="wc-article-figure">
  <img src="/books/codex-from-source/assets/startup-router-narrowing.png" alt="chapter 02 page hand-drawn architecture figure" loading="lazy" decoding="async" />
  <figcaption>启动路径把宽泛命令逐层收窄成类型化运行时入口：wrapper、Rust CLI、命令路由，再到具体 surface。</figcaption>
</figure>
...
<DiagramFlow spec={startupChain} lang="zh" client:visible />
...
<DiagramFlow spec={routerToFoundation} lang="zh" client:visible />
