...
import { toolSpecPlanner } from "../../../src/data/diagrams/codex-from-source/chapter-09";

# 第 9 章：工具规格、路由与分发

<ToolDispatchMap lang="zh" client:visible />

> **阅读契约：** 用本章跟踪工具如何从 model-visible spec 到 runtime handler。跟住 routing、registry lookup、orchestration，并记住 spec 从来不是执行权限。

<figure class="wc-article-figure">
  <img src="/books/codex-from-source/assets/tool-routing-dispatch-map.png" alt="chapter 09 page hand-drawn architecture figure" loading="lazy" decoding="async" />
  <figcaption>模型可见的 tool spec 只是第一道门；routing、registry lookup、orchestration 和 result shaping 决定真正的副作用。</figcaption>
</figure>
...
<DiagramFlow spec={toolSpecPlanner} lang="zh" client:visible />
