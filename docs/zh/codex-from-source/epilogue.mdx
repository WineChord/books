...
<figure class="wc-article-figure">
  <img src="https://cdn.jsdelivr.net/gh/WineChord/typora-images/img/transferable-agent-runtime-lessons-df64b03.webp" alt="可迁移 agent runtime 经验图：从 typed contracts 穿过 authority、rollout、projections、memory 和 executable governance" loading="eager" decoding="async" />
  <figcaption>可迁移的教训不是照搬所有子系统，而是让 vocabulary、side effects、surfaces、facts 和 governance 保持显式。</figcaption>
</figure>
...
| 你自己的系统压力 | 值得借鉴的 Codex pattern | 被保护的不变量 |
| --- | --- | --- |
| 一个 planner 可以触发危险工作 | 让每个 side effect 都经过 typed request、policy check 和 observable outcome | planner 只建议，runtime 才授权 |
| 多个客户端需要同一个 agent | 建立一套 runtime contract，让 clients 成为它的 projections | clients 渲染状态，而不是发明状态 |
| 长工作需要恢复或审计 | 先记录 raw facts，再派生友好视图 | replay 和 debug 不依赖 UI 文本 |
| 扩展要增长但不能接管系统 | 分离 packaging、provenance、routing 和 trust planes | 可选 capability 不变成 ambient authority |
| 架构规则总在 review 中漂移 | 把最高风险规则编码成 executable checks | 重要边界先失败，而不是悄悄腐蚀 |
