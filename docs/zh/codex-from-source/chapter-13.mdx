...
> **阅读契约：** 用本章把 containment 具体化。跟住 sandbox selection、network policy、platform boundaries，以及最终接触外部世界的 executor。

<figure class="wc-article-figure">
  <img src="https://cdn.jsdelivr.net/gh/WineChord/typora-images/img/sandbox-network-platform-boundary-df64b03.webp" alt="Sandbox 与网络边界图：展示 permission profiles、平台 transform、macOS Seatbelt、Linux Bubblewrap、Windows identities 和 managed proxy policy" loading="eager" decoding="async" />
  <figcaption>Containment 是运行时选择结果，由 requested action、permission profile、network policy、platform sandbox 和 executor backend 共同决定。</figcaption>
</figure>
...
**源码边界：** sandbox type selection 和 platform choice 是 **verified source**，锚点在 [`sandboxing/src/manager.rs`](https://github.com/openai/codex/blob/569ff6a1c400bd514ff79f5f1050a684dc3afde3/codex-rs/sandboxing/src/manager.rs#L23)；macOS lowering 是 **verified source**，锚点在 [`seatbelt.rs`](https://github.com/openai/codex/blob/569ff6a1c400bd514ff79f5f1050a684dc3afde3/codex-rs/sandboxing/src/seatbelt.rs#L31)；Linux execution 与 proxy routing 是 **verified source**，锚点在 [`linux-sandbox`](https://github.com/openai/codex/blob/569ff6a1c400bd514ff79f5f1050a684dc3afde3/codex-rs/linux-sandbox/src/main.rs#L4) 和 [`proxy_routing.rs`](https://github.com/openai/codex/blob/569ff6a1c400bd514ff79f5f1050a684dc3afde3/codex-rs/linux-sandbox/src/proxy_routing.rs#L169)；Windows setup 是 **verified source**，锚点在 [`windows-sandbox-rs`](https://github.com/openai/codex/blob/569ff6a1c400bd514ff79f5f1050a684dc3afde3/codex-rs/windows-sandbox-rs/src/setup.rs#L85)。凡是跨平台等价的表述，都只作为 **surrounding contract inference**；本章不声称不同操作系统能执行完全相同的 sandbox。
