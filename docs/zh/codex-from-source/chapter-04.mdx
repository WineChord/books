...
> **阅读契约：** 用本章进入词汇边界。跟住 operations、events、items、submissions 和 protocol schemas 如何成为 clients、sessions 与 replay 的共同语言。

<figure class="wc-article-figure">
  <img src="https://cdn.jsdelivr.net/gh/WineChord/typora-images/img/protocol-boundary-taxonomy-569ff6a1-v2.webp" alt="协议边界分类图：覆盖 submissions、operations、events、items、app-server messages、generated schemas 和 compatibility" loading="eager" decoding="async" />
  <figcaption>协议词汇是共享语言：operations 进入、submissions 排队、events 发出、items 渲染，schemas 保持客户端一致。</figcaption>
</figure>
