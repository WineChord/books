...
后端任务 workflow，有自己的身份、环境、attempts、diffs 和本地 apply
路径。这个分离非常关键。Local Codex turn 会流式输出模型结果，并在本地权限上下文
...
<figure class="sketch-figure">
  <img src="/books/figures/codex-from-source/excalidraw/chapter-21-01-zh.svg" alt="远程任务工作先跨过后端契约，随后本地 patch 引擎才会触碰工作树。" loading="lazy" />
  <figcaption>远程任务工作先跨过后端契约，随后本地 patch 引擎才会触碰工作树。</figcaption>
</figure>
...
branch，再查询 default branch，只有本地发现失败时才使用 generic default。重点是：cloud execution 应该锚定到用户能理解的 branch，而不是埋在 request
body 里的隐式本地猜测。
...
以及每个 attempt 可能携带的 diff。这就是 apply 和 diff commands 都接受 attempt selector 的原因。应用某个
attempt 时，应该使用那个 attempt 的 diff，而不是意外重新获取 base diff，
忽略用户选择。Task detail view 会把当前选中的 attempt 保存在 state 中，再
把这个 attempt 的 diff 和 text 拷贝到可见字段。更广泛的架构经验是：远程工作经常返回一个 result set，而不是单个 result。一旦 backend 可以产生多个候选，本地任何针对“答案”的操作都必须命名它选择
的是哪一个答案。
...
<figure class="sketch-figure">
  <img src="/books/figures/codex-from-source/excalidraw/chapter-21-02-zh.svg" alt="任务应用仍然是本地治理：后端提供候选 diff，但接触工作树的是 patch 引擎，它负责校验形状、基于当前状态预检并记录结果。" loading="lazy" />
  <figcaption>任务应用仍然是本地治理：后端提供候选 diff，但接触工作树的是 patch 引擎，它负责校验形状、基于当前状态预检并记录结果。</figcaption>
</figure>
...
Apply outcome 有三类：success、partial 和 error。Partial 不是装饰状态。它告诉客户端：有些 paths 被应用了，或产生了 conflict，另一些被 skipped。这会改变用户接下来应该如何检查工作树。

## Cloud TUI 是 Task Operator

Cloud TUI 不是一张薄表。它持有任务列表、环境弹窗、最佳尝试选择器、diff 覆盖层、prompt/messages 视图、后台补充、预检加载态、应用加载态和结果弹窗的状态。这些
复杂度是合理的，因为远程工作是异步的，而且可能有多个结果。

<figure class="sketch-figure">
  <img src="/books/figures/codex-from-source/excalidraw/chapter-21-concept-1-zh.svg" alt="Cloud TUI 是操作台：它选择任务和 attempt，但任务状态、身份和 patch material 仍由后端契约拥有。" loading="lazy" />
  <figcaption>Cloud TUI 是操作台：它选择任务和 attempt，但任务状态、身份和 patch material 仍由后端契约拥有。</figcaption>
</figure>

但是 TUI 仍然不拥有后端语义。它用后端契约加载任务
和尝试，用环境检测 helper 填充过滤器，用应用
预检保护本地变更，用尝试状态渲染正确的 diff 或 prompt。
UI 是 task system 的操作台，不是 task system 本身。这种区分让远程工作不会泄漏进本地 turn loop。本地会话仍然可以
使用模型流式输出、工具执行、审批和 rollout 持久化。
Cloud task 浏览和应用是独立流程，只在认证、Git 上下文和 patch 应用这些边界与本机相遇。
...
bill of materials，把一个运行中的 agent 绑定到后端任务。
...
<figure class="sketch-figure">
  <img src="/books/figures/codex-from-source/excalidraw/chapter-21-03-zh.svg" alt="Agent 身份避免远程任务权限变成环境权限：运行时密钥材料先签署任务范围内的断言，后端才授予访问。" loading="lazy" />
  <figcaption>Agent 身份避免远程任务权限变成环境权限：运行时密钥材料先签署任务范围内的断言，后端才授予访问。</figcaption>
</figure>
...
| --- | --- |
| session 和 turn loop 控制 streaming | 后端任务 status 控制 progress |
...
response 可能省略 direct task id，需要解密。这些不是 UI 错误，而是证明边界。实践规则很简单：不要让 remote success 直接等价于 local success，直到本地
边界完成检查。Completed task 只表示 backend 产生了 outcome；它不表示用户
当前 checkout 已经接受这个 outcome。

## 应用到实践

1. **Backend/local split。** 解决 cloud-task 含混问题；分开后端任务
   lifecycle 和本地 turn lifecycle；风险是把 remote success 当成 local
   mutation success。
...
Cloud tasks 把 Codex 扩展到单个本地 turn 之外，但没有模糊 runtime 边界。远程工作通过后端契约创建、列出、检查和授权；本地 mutation 仍然
经过本地 patch 检查。第 22 章会把同样的分离用于长期状态：memories 有用，是因为它们是受控侧通道，而不是偷偷变成另一份聊天历史。
