...
<figure class="wc-article-figure">
  <img src="https://cdn.jsdelivr.net/gh/WineChord/typora-images/img/ci-policy-governance-lanes-df64b03.webp" alt="CI 治理通道：连接策略检查、生成产物、Rust 测试、Bazel 校验和发布保护" loading="eager" decoding="async" />
  <figcaption>CI 通过分离 behavior tests、boundary linting、schema checks、artifact checks、risk policy 和 docs evidence 来保护架构。</figcaption>
</figure>
...
<figure class="wc-article-figure">
  <img src="https://cdn.jsdelivr.net/gh/WineChord/typora-images/img/generated-artifact-drift-state-machine-20260616.webp" alt="Generated artifact drift state machine：source type change、generator output、artifact diff、CI gate、fail regenerate path、review path 与 pass contract change" loading="lazy" decoding="async" width="1672" height="941" />
  <figcaption>Generated drift 只有在源码类型变化变成 visible diff、CI outcome、repair path 和可 review contract change 时，才真正成为治理。</figcaption>
</figure>
...
| 问题 | 第 25 章答案 |
| --- | --- |
| 用户请求现在在哪里？ | 它已经变成 repository change pressure：protocol edit、dependency edit、binary artifact、UI import、generated output 或 release workflow change。 |
| 什么数据结构携带它？ | workflow lanes、generated artifact checks、blob-size policy、dependency policy、boundary scripts、test matrices 和 release gates。 |
| 谁拥有下一步决策？ | CI 无法判断设计品味，但可以在 review 变成记忆劳动之前阻止破坏指定架构不变量的变更。 |
| 这里可能怎么失败？ | gate 过宽、policy scripts 过期、generated artifacts 缺失、TUI/core 直接 import、无法解释的大 blob，或失败时没有修复路径。 |
