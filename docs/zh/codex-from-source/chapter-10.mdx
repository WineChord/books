...
<ExecutionBackendBoard lang="zh" client:visible />

> **阅读契约：** 用本章把 shell 和文件系统访问读成有边界能力。把 exec-server boundary、command policy、filesystem mutation 和 result reporting 放在一起看。

<figure class="wc-article-figure">
  <img src="/books/codex-from-source/assets/shell-exec-filesystem-boundary.png" alt="chapter 10 page hand-drawn architecture figure" loading="lazy" decoding="async" />
  <figcaption>Shell 和文件系统访问之所以可控，是因为命令经过 parse、policy、approval、sandbox、execution、sequencing 和 recording。</figcaption>
</figure>
...
<DiagramFlow spec={shellExecutionChain} lang="zh" client:visible />
