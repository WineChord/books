...
本书是对公开
[OpenAI Codex 仓库](https://github.com/openai/codex) 的源码等价导读。
...
| 活动 | 目的 |
| --- | --- |
| 自检题 | 只根据正文回答。如果答不出来，说明章节还解释不够。 |
| 可选源码实验 | 只有在需要行级验证或审计证据时，才打开固定源码链接。 |

建议先读[阅读地图](reader-map)。它提供 30 分钟路径、2 小时路径和可选源码审计路径。本书的目标不是把你推回仓库，而是把仓库中重要的知识转成可直接阅读的书。
