...
<figure class="sketch-figure">
  <img src="/books/figures/codex-from-source/excalidraw/chapter-05-01-zh.svg" alt="运行时栈把客户端入口的权限逐层收窄，经 ThreadManager 与 CodexThread 进入提交队列、会话状态、调度任务和工具副作用。" loading="lazy" />
  <figcaption>运行时栈把客户端入口的权限逐层收窄，经 ThreadManager 与 CodexThread 进入提交队列、会话状态、调度任务和工具副作用。</figcaption>
</figure>
...
状态、目标状态、review 状态和各种 service handle。这层拆分很关键。客户端不应该直接改调度器内部状态。客户端提交一个
...
审批策略、权限配置、初始消息和其他已解析设置。

<p class="sketch-intro">第一个事件图锚定后续所有恢复路径：每个框都是启动流程不能沦为副作用的原因。</p>
<figure class="sketch-figure">
  <img src="/books/figures/codex-from-source/excalidraw/chapter-05-concept-1-zh.svg" alt="setup 事件先为所有客户端固定事件流锚点，随后持久历史、回放前缀、分层会话状态和抽取出的元数据各自承担不同职责。" loading="lazy" />
  <figcaption>setup 事件先为所有客户端固定事件流锚点，随后持久历史、回放前缀、分层会话状态和抽取出的元数据各自承担不同职责。</figcaption>
</figure>
...
一次 turn 记录一个新 item 时，runtime 必须判断哪些 surface 需要知道它。一个工具结果可能是模型可见的、可 replay 的、客户端可渲染的、analytics
...
<figure class="sketch-figure">
  <img src="/books/figures/codex-from-source/excalidraw/chapter-05-02-zh.svg" alt="start、resume、fork 和 rollback 都围绕选定的 replay 前缀打开持久化、重建 live session，并在得到的 thread identity 下继续追加未来条目。" loading="lazy" />
  <figcaption>start、resume、fork 和 rollback 都围绕选定的 replay 前缀打开持久化、重建 live session，并在得到的 thread identity 下继续追加未来条目。</figcaption>
</figure>
...
compaction 和未完成 turn。这就是为什么 turn context snapshot 必须持久化。它告诉后来的 session：
当时的工作目录、权限、模型设置、网络策略、memory mode 和环境选择是什么。没有这些上下文，replay 就只剩文本，没有执行语义。
...
<figure class="sketch-figure">
  <img src="/books/figures/codex-from-source/excalidraw/chapter-05-concept-2-zh.svg" alt="分层会话状态把持久线程承诺与配置、可变运行时数据、活动 turn 工作、邮箱输入和存储句柄分开。" loading="lazy" />
  <figcaption>分层会话状态把持久线程承诺与配置、可变运行时数据、活动 turn 工作、邮箱输入和存储句柄分开。</figcaption>
</figure>
...
当前 live turn、下一次 turn，还是只是客户端视图？”这些承诺完全不同。如果 runtime 分不清，迟早会丢输入、重复工具结果，或者让 resume 变得不一致。
...
projection 的价值在于可查询；它的安全性来自背后仍然保留持久 rollout。当列表行缺失或过期时，系统可以扫描对应 rollout 的头部，或者 replay
