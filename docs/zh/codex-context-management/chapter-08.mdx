...
<figure class="wc-article-figure">
  <img src="https://cdn.jsdelivr.net/gh/WineChord/typora-images/img/semantic-trace-boundary.png" alt="Semantic trace boundary：compact input、checkpoint install、future sampling、trace reducers、runtime ledger 与 client surfaces 的分界" loading="lazy" decoding="async" />
...
1. **Runtime-Owned Display：** 客户端可以渲染上下文事实，但不能拥有它；迁移时让 UI 状态从 runtime 事件派生；避免只存在于 UI、模型从未看到的上下文替代 runtime state。
2. **Connection-Scoped Replay：** 历史上下文事实只回放给正在附加的观察者；迁移到 resume 客户端；标记 replay 事件，避免它们被当成新 live 事件。
3. **Attribution Fallback：** 恢复 usage 时优先按 id，其次按位置归属；迁移到重建时间线；避免重新生成的 id 破坏 UI 状态。
4. **Mode as Context：** 改变行为的交互模式视为模型可见上下文；迁移时 diff 模式状态；不要把改变行为的 transport 标志藏在 prompt 之外。
5. **Semantic Trace Boundary：** 把上下文重写当作 install 事件 trace；迁移时区分 compaction input 与后续 sampling input；避免 observability 把多阶段折成一团。
