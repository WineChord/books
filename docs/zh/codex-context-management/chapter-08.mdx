...
<DiagramFlow spec={tokenFanout} lang="zh" client:visible />
...
<DiagramSequence spec={replayAttribution} lang="zh" client:visible />
...
<figure class="wc-article-figure">
  <img src="/books/codex-context-management/assets/semantic-trace-boundary.png" alt="Semantic trace boundary：compact input、checkpoint install、future sampling、trace reducers、runtime ledger 与 client surfaces 的分界" loading="lazy" decoding="async" />
  <figcaption>Trace 有价值，是因为它没有把 compact input、checkpoint install 与后续 sampling 混成一个模糊事件；每个阶段都可以单独审计。</figcaption>
</figure>
