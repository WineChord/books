...
<figure class="wc-article-figure">
  <img src="/books/codex-context-management/assets/rollout-reconstruction-boundaries.png" alt="Rollout reconstruction 逆向扫描 checkpoint、rollback marker、agent envelope、user turn 与 assistant turn 的边界" loading="lazy" decoding="async" />
  <figcaption>Resume 与 fork 都是投影问题：raw rollout 仍保持 append-only，重建过程选择最新存活 checkpoint、应用 rollback marker，并尊重 agent 触发的边界。</figcaption>
</figure>
