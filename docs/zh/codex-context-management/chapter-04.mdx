...
<FragmentComposer lang="zh" client:visible />
...
<DiagramFlow spec={fragmentLanes} lang="zh" client:visible />

三条泳道避免了一个常见 bug：运行时状态被走多条不一致的代码路径渲染。一旦有了 fragment，任何想注入相同事实的子系统都会复用同一个 renderer。

<figure class="wc-article-figure">
  <img src="/books/codex-context-management/assets/typed-fragment-catalog-board.png" alt="Typed fragment catalog：initial context、settings diff 与 ContextualUserFragment 家族的架构清单" loading="lazy" decoding="async" />
...
<DiagramFlow spec={settingsDiff} lang="zh" client:visible />
