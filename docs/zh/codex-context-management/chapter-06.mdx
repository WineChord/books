...
<figure class="wc-article-figure">
  <img src="https://cdn.jsdelivr.net/gh/WineChord/typora-images/img/compaction-checkpoint-install.png" alt="Codex compaction checkpoint 安装流程：旧历史经过 compact producer 与 runtime install 变成 replacement history" loading="lazy" decoding="async" />
...
1. **Compaction Checkpoint：** 把压缩输出作为 replacement history 安装；迁移时存压缩后 prompt 起点；避免只摘要但无法重建状态的设计。
2. **Placement Mode：** 让 turn 前与 turn 中 compaction 的上下文放置显式化；迁移时给放置策略命名；避免"一刀切"摘要插入。
3. **Hooked Forgetting：** 在语义历史重写两侧运行策略 hook；迁移时把 compaction 当作状态变更；拒绝后台无声遗忘。
4. **Provider-Owned Work, Runtime-Owned Install：** 让 provider 生产 compacted 历史，但过滤与安装留在本地；迁移到外部摘要器时保持这条边界；不要把远端输出当作"已经安全"。
5. **Token Recompute：** 替换后重新计算 usage；迁移时让旧计数失效；避免 UI 或 compaction 阈值仍以压缩前总数为基础。
