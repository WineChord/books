...
<figure class="wc-article-figure">
  <img src="/books/codex-context-management/assets/history-ledger-normalization.png" alt="History ledger 从结构化证据记录、输出裁剪、provider 不变量归一化，到模型可用 prompt view 的流程" loading="eager" decoding="async" />
  <figcaption>账本先记录结构化证据，再派生 provider-safe prompt view；rollback 与 compaction 可以清除过期 baseline，而不污染持久记录。</figcaption>
</figure>
