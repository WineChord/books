...
<figure class="wc-article-figure">
  <img src="/books/codex-context-management/assets/context-runtime-overview.png" alt="Codex 上下文的六个 owner：history ledger、turn envelope、typed fragments、optional planes、compaction 与 replay evidence 共同围绕 prompt projection" loading="eager" decoding="async" />
  <figcaption>整本书可以先读成一张所有权地图：持久证据、turn-local 事实、渲染片段、可选平面、checkpoint 与 replay 都汇入 prompt projection，但它们从不合并成同一种东西。</figcaption>
</figure>
