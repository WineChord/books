...
Accepted 平均耗时，以及每 10 行非空代码耗时。页面用一个紧凑的推荐面板
按启发式不熟练分排序：提交失败、失败率、按代码量归一后的 Accepted 耗时、
手动“需思考”状态和遗忘曲线会提高分数；近期连续 Accepted 和手动“熟练”
会降低分数。分数最高的题会成为“猜你想写”，排序靠前的题可以直接从推荐
面板打开。这个排序只作为浏览器本地的启发式建议。
