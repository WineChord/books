...
做成发灰、发虚的辅助说明。题面和约束句统一使用更大的楷体阅读字体，并保持
普通视觉字重。少数官方题面没有单独列出数据范围的题，例如部分数据库题，会
明确提示以题面中的表结构或输入定义为准。
...
练习区；不在本书里的相关题跳到力扣中国区，并保留力扣返回的付费标记。所有
预览触点都默认以小按钮或短文本紧贴题名，不额外撑高行高。
...
默认页面字体在 Apple 设备上使用系统黑体，在其它设备上使用楷体；用户也可以
切回原本衬线字体或系统黑体，这个设置只影响练题页面 UI，不改变代码编辑器
字体。页面底部还提供编辑器主题、字体、字号
...
焦点、不得要求用户确认、不得阻塞继续输入，也不得改变页面布局。奖励强度
应随普通连击、里程碑连击和刷新最高纪录逐层升级；用户开启 reduced motion
时，应退化为更安静的静态提示，而不是大量粒子动画。
