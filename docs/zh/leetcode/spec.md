...
通过本页完整提交到力扣时，页面还会写入一份独立的本地提交统计。它不导入
力扣历史提交记录，只记录这个功能上线后从本地编辑器发起的提交。每道题会
记录提交次数、Accepted 次数、提交出错次数、最近代码量、Accepted 耗时、
Accepted 平均耗时，以及每 10 行非空代码耗时。页面用一个紧凑的推荐按钮
承载 hover/focus 浮层，按启发式不熟练分排序：提交失败、失败率、按代码量
归一后的 Accepted 耗时、手动“需思考”状态和遗忘曲线会提高分数；近期连续
Accepted 和手动“熟练”会降低分数。分数最高的题会成为“猜你想写”，排序
靠前的题可以直接从浮层打开。这个排序只作为浏览器本地的启发式建议。
