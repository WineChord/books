...
光标位置。练题导航也要键盘优先：`Option+J` 或 `Option+Down` 打开并聚焦
...
耗时。页面用一个紧凑的推荐按钮承载 hover/focus 浮层，按 0 到 100 的启发式
不熟练分排序：缺少完整 Accepted、提交失败、样例运行失败、按代码量归一后的
Accepted 耗时偏慢、手动“需思考”状态和遗忘曲线会提高分数；近期连续
Accepted、样例通过和手动“熟练”会降低分数。样例运行失败会单独统计，并且
权重低于完整提交失败。失败率会做平滑，失败次数按边际递减增长，遗忘曲线
会逐渐接近上限而不是无限累加。页尾有一个折叠说明解释打分模型和排序兜底
规则。分数最高的题会成为“猜你想写”，排序靠前的题可以直接从浮层打开。
这个排序只作为浏览器本地的启发式建议。
