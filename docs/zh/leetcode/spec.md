...
达式，约束之间在所有语言里都使用带空隙的半角分号；中文标签等纯中文说明
仍按中文标点处理。约束文字必须使用接近题面正文的清晰字重和颜色，不得做成
发灰、发虚的辅助说明。少数官方题面没有单独列出数据范围的题，例如部分数据
库题，会明确提示以题面中的表结构或输入定义为准。
...
为行内数学；普通叙述数字若不是公式或范围的一部分，则保留文本。页面只
读取这份已转义的生成 HTML，并全局引入 KaTeX CSS。发布题面或数学相关改动
前，`npm run check:leetcode-math` 必须通过。
