...
case tab。运行结果返回后，每个 case 还可以展示提交代码的实际输出、预期
输出和 stdout；如果力扣只返回一整段 blob，页面会把它保留为独立调试行，
不强行猜测归属到错误 case。运行样例只有在力扣返回的所有 testcase 计数都
通过时才算通过。完整提交返回失败用例时，页面会在状态行下方展示失败用例、
