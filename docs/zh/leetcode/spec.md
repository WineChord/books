...
case tab。运行样例只有在力扣返回的所有 testcase 计数都通过时才算通过。
完整提交返回失败用例时，页面会在状态行下方展示失败用例、实际输出、期望输出，
以及编译或运行错误诊断，并把该失败用例保存为同题下次运行样例的补充用例；
如果同样归一化后的用例已经保存，则不会重复添加。补充用例会在下一个北京时间
自然日清空，所以每天练习都会重新从官方默认样例开始。
页面底部还提供常驻的 GitHub 仓库、扩展目录、反馈 issue 和页面规格链接。
扩展安装说明默认折叠，讲清楚如何从本仓库加载或重新加载这个未打包的 Chrome
扩展；练习编辑器的扩展状态区会直接链接到这段说明。
